...
export default function CareerApplicationsPage() {
  ...
  const fetchApplications = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/career-applications?t=${Date.now()}`, {
        headers: {
      ...
      if (!res.ok) throw new Error("Failed to load career applications from server.");
  ...
  const handleUpdateStatus = async (id: string, newStatus: string) => {
    ...
    try {
      const apiBase = process.env.NEXT_PUBLIC_API_URL || "http://localhost:5001/api";
      const res = await fetch(`${apiBase}/career-applications/${id}`, {
        method: "PUT",
        headers: {
  ...
  const handleOpenEmailModal = (app: CareerApplication) => {
    setEmailSubject(`Re: Career Application for ${app.areaOfInterest}`);
    setEmailBody(`Dear ${app.name},\n\nThank you for your application for the ${app.areaOfInterest} department at Supremo. We have received your application.\n\nBest Regards,\nSupremo HR Team`);
  ...
  const handleSendEmail = async (id: string) => {
    ...
    try {
      const apiBase = process.env.NEXT_PUBLIC_API_URL || "http://localhost:5001/api";
      const res = await fetch(`${apiBase}/career-applications/${id}/send-email`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${user.token}`,
  ...
  const detailCareer = applications.find((app) => app._id === selectedId);

  // Helper to determine if resume is PDF or Image
  const isResumePdf = (resumeUrl: string) => {
    return resumeUrl.toLowerCase().endsWith(".pdf") || resumeUrl.includes("/uploads/") && !resumeUrl.toLowerCase().match(/\.(png|jpe?g|gif|webp)$/);
  };

  if (loading) {
    return (
      <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
        <AdminHeader title="Career Applications" breadcrumb={[{ label: "Career Applications" }]} />
        <main style={{ flex: 1, padding: "28px", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
            ...
            <div style={{ fontSize: 13, color: "var(--muted)" }}>Loading applications...</div>
          </div>
      ...
      <main style={{ flex: 1, padding: "28px", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div className="adm-cols-4" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }}>
          {[
            ...
            { label: "Shortlisted", value: counts.Active, color: "var(--ok)" },
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: 4, background: "var(--paper)", border: "1px solid var(--line)", borderRadius: "var(--r-sm)", padding: "4px", overflowX: "auto", maxWidth: "100%" }}>
            {FILTER_OPTIONS.map((f) => (
              ...
              >
                {f === "Active" ? "Shortlisted" : f} {f !== "All" && <span style={{ opacity: 0.7, fontSize: 10 }}>({counts[f as keyof typeof counts]})</span>}
              </button>
            ))}
          </div>
        </div>
        ...
        {/* Split View */}
        <div style={{ display: "grid", gridTemplateColumns: detailCareer ? "1fr 440px" : "1fr", gap: 20, alignItems: "start" }}>
          ...
          <div className="adm-table-scroll" style={{ background: "var(--paper)", borderRadius: "var(--r-md)", border: "1px solid var(--line-2)", boxShadow: "var(--sh-sm)", overflow: "hidden" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1.8fr 1.5fr 1.2fr 90px 90px", padding: "12px 20px", background: "var(--paper-2)", borderBottom: "1px solid var(--line-2)", gap: 12 }}>
              {["Applicant", "Area of Interest", "Contact Phone", "Date", "Status"].map((h) => (
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: "var(--muted)" }}>{app.email}</div>
                    </div>
                  </div>
                  <div style={{ fontSize: 13, color: "var(--ink)", fontWeight: 500 }}>{app.areaOfInterest}</div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ fontSize: 11, fontWeight: 600, padding: "3px 9px", borderRadius: "var(--r-pill)", background: ss.bg, color: ss.color }}>
                      {app.status === "Active" ? "Shortlisted" : app.status}
                    </span>
                  </div>
                </div>
              );
            ...
            <div style={{ background: "var(--paper)", borderRadius: "var(--r-md)", border: "1px solid var(--line-2)", boxShadow: "var(--sh-sm)", overflow: "hidden", position: "sticky", top: 88 }}>
              <div style={{ padding: "18px 20px 14px", borderBottom: "1px solid var(--line-2)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div>
                  ...
                  <span style={{ fontSize: 10, textTransform: "uppercase", color: "var(--blue-600)", fontWeight: 700 }}>
                    Submitted On: {new Date(detailCareer.createdAt).toLocaleDateString()}
                  </span>
                </div>
                <button
              ...
              <div style={{ padding: "20px", display: "flex", flexDirection: "column", gap: 16 }}>
                <div>
                  ...
                  <div style={{ fontSize: 14, fontWeight: 600, color: "var(--ink)" }}>{detailCareer.name}</div>
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 600, color: "var(--ink)" }}>{detailCareer.phone}</div>
                  </div>
                  <div>
                    ...
                    <div style={{ fontSize: 13, color: "var(--ink)", wordBreak: "break-all" }}>{detailCareer.email}</div>
                  </div>
                  <div style={{ gridColumn: "1 / -1" }}>
                    <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", color: "var(--muted)", display: "block", marginBottom: 4 }}>Area of Interest</span>
                    <div style={{ fontSize: 13, color: "var(--ink)", fontWeight: 600 }}>{detailCareer.areaOfInterest}</div>
                  </div>
                </div>

                {detailCareer.message && (
                  <div>
                    <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", color: "var(--muted)", display: "block", marginBottom: 4 }}>Cover Message</span>
                ...
                {/* Resume display */}
                <div>
                  <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", color: "var(--muted)", display: "block", marginBottom: 6 }}>Resume Attachment</span>
                  {isResumePdf(detailCareer.resume) ? (
                    <div style={{ padding: "16px", background: "var(--paper-2)", borderRadius: "var(--r-sm)", border: "1px solid var(--line)", display: "flex", flexDirection: "column", gap: 12, alignItems: "center", justifyContent: "center" }}>
                      <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="var(--blue-600)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
                        <polyline points="14 2 14 8 20 8" />
                        <line x1="16" y1="13" x2="8" y2="13" />
                        <line x1="16" y1="17" x2="8" y2="17" />
                      </svg>
                      <span style={{ fontSize: 12, color: "var(--ink)", fontWeight: 600 }}>PDF Document Resume</span>
                      <a
                        href={`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5001/api"}/media/download?url=${encodeURIComponent(detailCareer.resume)}`}
                        target="_blank"
                        rel="noreferrer"
                        style={{
                          height: 32, padding: "0 14px", background: "var(--blue-600)", border: "none", borderRadius: "var(--r-sm)",
                          display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6,
                          fontSize: 12, fontWeight: 600, color: "#fff", textDecoration: "none", transition: "background .15s"
                        }}
                    ...
                    <div>
                      <a href={detailCareer.resume} target="_blank" rel="noreferrer" style={{ display: "block", position: "relative", width: "100%", height: 160, borderRadius: "var(--r-sm)", border: "1px solid var(--line)", overflow: "hidden", cursor: "zoom-in", marginBottom: 10 }}>
                        <LazyImage src={detailCareer.resume} alt="Resume thumbnail" style={{ width: "100%", height: "100%", objectFit: "contain", background: "var(--paper-2)" }} />
                      </a>
                      <a
                        href={`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5001/api"}/media/download?url=${encodeURIComponent(detailCareer.resume)}`}
                        target="_blank"
                        rel="noreferrer"
                        style={{
                          height: 32, width: "100%", background: "var(--paper-2)", border: "1px solid var(--line)", borderRadius: "var(--r-sm)",
                          display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6,
                          fontSize: 12, fontWeight: 600, color: "var(--slate)", textDecoration: "none", transition: "background .15s"
                        }}
                        onMouseEnter={(e) => { e.currentTarget.style.background = "var(--line)"; }}
                        onMouseLeave={(e) => { e.currentTarget.style.background = "var(--paper-2)"; }}
                      >
                        Download Image Resume
                      </a>
                    </div>
                  )}
                </div>
                ...
                <div>
                  <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", color: "var(--muted)", display: "block", marginBottom: 8 }}>
                    Action Status ({detailCareer.status === "Active" ? "Shortlisted" : detailCareer.status})
                  </span>
                  
                  <button
                    disabled={updating}
                    onClick={() => handleOpenEmailModal(detailCareer)}
                    style={{
                  ...
                  {detailCareer.status === "Active" ? (
                    ...
                    <>
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        <button
                          disabled={updating}
                          onClick={() => handleUpdateStatus(detailCareer._id, "Active")}
                          style={{
                        ...
                        <button
                          disabled={updating}
                          onClick={() => handleUpdateStatus(detailCareer._id, "Rejected")}
                          style={{
                      ...
                      {detailCareer.status !== "New" && (
                        <button
                          disabled={updating}
                          onClick={() => handleUpdateStatus(detailCareer._id, "New")}
                          style={{
                      ...
                      <button
                        disabled={updating}
                        onClick={() => handleDeleteApplication(detailCareer._id)}
                        style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                ...
                <div style={{ fontSize: 12, color: "var(--muted)" }}>
                  To: <span style={{ fontWeight: 600, color: "var(--ink)" }}>{detailCareer.name}</span> &lt;{detailCareer.email}&gt;
                </div>
              </div>
              <button
            ...
            <div style={{
              ...
              <button
                ...
                onClick={() => handleSendEmail(detailCareer._id)}
                style={{
                  height: 38,
                  padding: "0 20px",
                  borderRadius: "var(--r-sm)",
