...
export default function TermsEditorPage() {
  ...
  const fetchTermsContent = () => {
    setLoading(true);
    setErrorMsg("");
    fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5001/api"}/terms`)
      .then((res) => {
        if (!res.ok) throw new Error("Failed to retrieve terms content.");
        return res.json();
      })
      .then((data) => {
        setS({
      ...
      .catch((err) => {
        ...
        setErrorMsg("Failed to load Terms & Conditions data from the server.");
        setLoading(false);
      });
  };
  ...
  const saveTermsContent = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5001/api"}/terms`, {
        method: "PUT",
        headers: {
      ...
      alert("Error saving terms content.");
    }
  };
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
        <AdminHeader title="Terms & Conditions Settings" breadcrumb={[{ label: "Terms Settings" }]} />
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh", paddingBottom: 80 }}>
      ...
      <main style={{ flex: 1, padding: "28px", display: "flex", flexDirection: "column", gap: 24, maxWidth: 1000 }}>
        ...
        <SectionCard title="General Info" description="Modify the primary title and intro text block displayed at the top of the Terms Page.">
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <TextArea
              ...
              placeholder="Provide a description of these terms..."
              rows={3}
            />
          </div>
        </SectionCard>
        ...
        {/* Sections list */}
        <SectionCard title="Terms Sections" description="Manage the individual legal sections and clauses for Terms & Conditions.">
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            {!s.sections || s.sections.length === 0 ? (
              <div style={{ padding: "16px", background: "var(--paper-2)", borderRadius: "var(--r-sm)", border: "1px dashed var(--line)", textAlign: "center", color: "var(--muted)", fontSize: 13 }}>
                No terms sections configured. Add some below.
              </div>
            ) : (
                ...
                {s.sections.map((sect, idx) => (
                  <div key={idx} style={{ padding: "20px", background: "var(--paper)", borderRadius: "var(--r-md)", border: "1px solid var(--line)", display: "flex", flexDirection: "column", gap: 12 }}>
                    ...
                    <Field
                      ...
                      placeholder="e.g. 1. Intellectual Property"
                    />
                    <TextArea
            ...
            <div style={{ borderTop: "1px solid var(--line-2)", paddingTop: 20, marginTop: 12 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <Field
                  ...
                  placeholder="e.g. 6. User Conduct"
                />
                <TextArea
        ...
        <SaveBar
          saved={saved}
          onSave={saveTermsContent}
          onReset={fetchTermsContent}
        />
      </main>
    </div>
  );
