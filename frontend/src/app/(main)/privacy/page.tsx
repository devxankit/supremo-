...
const DEFAULT_PRIVACY: LegalContent = {
  ...
  intro: "We value your privacy. This policy details how we collect, store, and handle your personal data when you use our website or services.",
  sections: []
};

export default function PrivacyPage() {
  const [data, setData] = useState<LegalContent>(DEFAULT_PRIVACY);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const apiBase = process.env.NEXT_PUBLIC_API_URL || "http://localhost:5001/api";
    fetch(`${apiBase}/privacy`)
      .then((res) => {
      ...
      .then((content) => {
        setData({
          title: content.title || DEFAULT_PRIVACY.title,
          intro: content.intro || DEFAULT_PRIVACY.intro,
          sections: content.sections || DEFAULT_PRIVACY.sections
        });
        setLoading(false);
      })
      .catch((err) => {
        ...
        <div className="container" style={{ maxWidth: 800 }}>
          {data.sections.length === 0 ? (
            <div style={{ textAlign: "center", color: "var(--muted)", fontSize: 15 }}>
              No privacy policy sections configured.
            </div>
          ) : (
